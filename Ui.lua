...
local Themes = {
	...
	Dark = {
		...
		AcrylicNoise = 0.6,
		TitleBarLine = Color3.fromRGB(120, 90, 255),
...
local function CloseOpen()
	...
	ScreenGui.Name = "CtrlOverlay"
	ScreenGui.Parent = RunService:IsStudio() and LocalPlayer.PlayerGui or (_getGuiParent())
	...
	Close_ImageButton.Image = "rbxassetid://94193921667843"
...
local Close_ImageButton
task.defer(function()
	Close_ImageButton = CloseOpen()
end)
-- Ensure Close_ImageButton exists before TitleBar code runs (TitleBar is built after window)
local function GetCloseButton()
	if not Close_ImageButton then
		Close_ImageButton = CloseOpen()
	end
	return Close_ImageButton
end
...
function GroupMotor:getValue()
	if not self._cachedValues then self._cachedValues = {} end
	local values = self._cachedValues
...
function Creator.AddThemeObject(Object, Properties)
	...
	-- Apply only this object's theme properties (avoids O(n^2) full-registry scan)
...
function Creator.GetThemeProperty(Property)
	...
	return Themes["Default_AH"][Property]
end
...
local GUI = New("ScreenGui", {
	Name = "ScreenContainer_fpdsid",
	Parent = RunService:IsStudio() and LocalPlayer.PlayerGui or (_getGuiParent()),
})
...
-- direct references already defined above; no unpack needed
...
function AcrylicPaint()
	...
	return function(props)
		...
		AcrylicPaint.Frame = New("Frame", {
			...
			New("ImageLabel", {
				Image = "rbxassetid://94193921667843",
				ScaleType = "Slice",
...
-- Lazy acrylic: factories are only invoked when actually used
local _acrylicBlurFactory = nil
local _acrylicPaintFactory = nil
local Acrylic = {
	CreateAcrylic = createAcrylic,
}
Acrylic.AcrylicBlur = function(...)
	if not _acrylicBlurFactory then
		_acrylicBlurFactory = AcrylicBlur()
	end
	return _acrylicBlurFactory(...)
end
Acrylic.AcrylicPaint = function(...)
	if not _acrylicPaintFactory then
		_acrylicPaintFactory = AcrylicPaint()
	end
	return _acrylicPaintFactory(...)
end
...
Components.Textbox = (function()
	...
	return function(Parent, Acrylic)
		...
		-- deferred first update so TextBounds is valid
		task.defer(Update)
	...
	return function(Config)
		...
		TitleBar.CloseButton = BarButton(Components.Assets.Close, UDim2.new(1, -4, 0, 4), TitleBar.Frame, function()
			Library.Window:Dialog({
				Title = "Confirmation !",
				Content = "Do you want to destroy this window ? ",
				Buttons = {
					{
						Title = "Yes",
						Callback = function()
						game:GetService("StarterGui"):SetCore("SendNotification", {
							Title = "Goodbye";
							Text = "TBaii :3";
							Icon = "rbxassetid://94193921667843";
							Duration = 2;
							})
							Library:Destroy()
						end,
				...
				Creator.AddSignal(TransparencySlider.InputBegan, function(Input)
					if Input.UserInputType == Enum.UserInputType.MouseButton1 then
						...
						_conn = RunService.RenderStepped:Connect(function()
							...
							Transparency = 1 - ((math.clamp(Mouse.Y, MinY, MaxY) - MinY) / (MaxY - MinY))
							Display()
						end)
					end
				end)
...
local SaveManager = {} do
	...
	function SaveManager:Save(Name)
		...
		local success, encoded = pcall(Http.JSONEncode, Http, data)
		if not success then
...
local InterfaceManager = {} do
	InterfaceManager.Folder = "ActHub"
	...
	function InterfaceManager:SaveSettings()
		writefile(self.Folder .. "/options.json", Http:JSONEncode(InterfaceManager.Settings))
	end
	...
	function InterfaceManager:LoadSettings()
		...
		if isfile(path) then
			...
			local success, decoded = pcall(Http.JSONDecode, Http, data)
...
Library.MiniWindow = Components.MiniWindow
